...
(function () {
    ...
    function Drawer(scale, container, mode) {
        ...
        this.on_resize = function () {
            ...
            // Fallback: if wrapper has no size, use container's size or default
            if (new_width === 0 || new_height === 0) {
                new_width = container.clientWidth || 600;
                new_height = container.clientHeight || 400;
...

{
                ...
                ctx.fillText("Î¸â‚ = " + (Math.abs(a) * 180 / Math.PI).toFixed(1) + "Â°", 0, 0);
                ctx.restore();
                ...
                if (Math.abs(sin2) < 1) {
                    ...
                    ctx.fillText("Î¸â‚‚ = " + (Math.abs(a2) * 180 / Math.PI).toFixed(1) + "Â°", 0, 0);
                    ctx.restore();
                ...
                ctx.textAlign = "left";
                ctx.fillStyle = blue_style;
                ctx.fillText("nâ‚ = " + n1.toFixed(2), -width * 0.45, -height * 0.45 + 10);

                ctx.fillStyle = yellow_style;
                ctx.fillText("nâ‚‚ = " + n2.toFixed(2), -width * 0.45, height * 0.45);
    ...
    document.addEventListener("DOMContentLoaded", function (event) {
        ...
        let slider_labels = {
            lens_detector_sl0: "Exposure time",
            lens_detector_rgbg_sl0: "Exposure time",
            lens_detector_rgb_sl0: "Exposure time",
            lens_rgb_filter_sl0: "Exposure time",
            lens_box_sl0: "Pinhole diameter",
            lens_box_sl1: "Sensor distance",
            lens_film_sl0: "Pinhole diameter",
            lens_film_sl1: "Sensor distance",
            lens_frustum_sl0: "Sensor distance",
            lens_field_sl0: "Sensor distance",
            lens_hole_solid_angle_sl0: "Pinhole diameter",
            lens_hole_sharpness_sl0: "Pinhole diameter",
            lens_film_exposure_sl0: "Pinhole diameter",
            lens_film_exposure_sl1: "Sensor distance",
            lens_subdiv_seg0: "Segments",
            lens_rotational_focal_sl0: "Focal distance",
            lens_lens_shape_sl0: "Curvature",
            lens_lens_shape_sl1: "Index of refraction",
            lens_field2_sl0: "Sensor distance",
            lens_field2_sl1: "Focal length",
            lens_lens_solid_angle_sl0: "Focal length",
            lens_lens_solid_angle_sl1: "Sensor distance",
            lens_focus_demo2_sl0: "Sensor distance",
            lens_focus_demo3_sl0: "Aperture",
            lens_focus_demo3_sl1: "Sensor distance",
            lens_cone_slice_sl0: "Object distance",
            lens_cone_angle_sl0: "Angle of the cone of light",
            lens_blades_sl0: "Aperture",
            lens_cone_hex_sl0: "Aperture",
            lens_cone_dof_sl0: "Aperture",
            lens_cone_dof_sl1: "Focal length",
            lens_cone_dof_sl2: "Object distance",
            lens_hero_sl0: "Aperture",
            lens_hero_sl1: "Focal length",
            lens_hero_sl2: "Sensor distance"
        };
        ...
        function maybeDrawer(id, mode) {
            let el = byId(id);
            if (!el)
                return null;
            return new Drawer(scale, el, mode);
        }

        function maybeSlider(id, callback, style_prefix, default_value, disable_click) {
            let el = byId(id);
            if (!el)
                return null;
            if (slider_labels[id]) {
                el.setAttribute("data-label", slider_labels[id]);
                el.setAttribute("aria-label", slider_labels[id]);
            }
            return new Slider(el, callback, style_prefix, default_value, disable_click);
        }

        function maybeSegmentedControl(id, callback, values) {
            let el = byId(id);
            if (!el)
                return null;
            return new SegmentedControl(el, callback, values);
        }


        maybeDrawer("lens_scene", "scene");
        maybeDrawer("lens_bare_film", "bare_film");

        detector = maybeDrawer("lens_detector", "detector");
        detector_slider = maybeSlider("lens_detector_sl0", function (x) {
            if (detector)
                detector.set_arg0(x);
        }, undefined, 0);


        let detector_rgbg = maybeDrawer("lens_detector_rgbg", "detector_rgbg");
        maybeSlider("lens_detector_rgbg_sl0", function (x) {
            if (detector_rgbg)
                detector_rgbg.set_arg0(x);
        }, undefined, 0);

        let detector_rgb = maybeDrawer("lens_detector_rgb", "detector_rgb");
        maybeSlider("lens_detector_rgb_sl0", function (x) {
            if (detector_rgb)
                detector_rgb.set_arg0(x);
        }, undefined, 0);

        let bayer = maybeDrawer("lens_bayer", "bayer");

        let rgb_filter = maybeDrawer("lens_rgb_filter", "rgb_filter");
        maybeSlider("lens_rgb_filter_sl0", function (x) {
            if (rgb_filter)
                rgb_filter.set_arg0(x);
        }, undefined, 0);


        maybeDrawer("lens_sensor_rays", "sensor_rays");
        maybeDrawer("lens_scene_rays", "scene_rays");


        let box = maybeDrawer("lens_box", "box");
        maybeSlider("lens_box_sl0", function (x) {
            if (box)
                box.set_arg0(x);
        });
        maybeSlider("lens_box_sl1", function (x) {
            if (box)
                box.set_arg1(x);
        });

        box_lens = maybeDrawer("lens_box_lens", "box_lens");
        maybeSlider("lens_box_lens_sl0", function (x) {
            if (box_lens)
                box_lens.set_arg0(x);
        });
        maybeSlider("lens_box_lens_sl1", function (x) {
            if (box_lens)
                box_lens.set_arg1(x);
        });


        let box_ap = maybeDrawer("lens_box_aperture", "box_aperture");
        maybeSlider("lens_box_aperture_sl0", function (x) {
            if (box_ap)
                box_ap.set_arg0(x);
        });
        maybeSlider("lens_box_aperture_sl1", function (x) {
            if (box_ap)
                box_ap.set_arg1(x);
        });


        let film = maybeDrawer("lens_film", "film");
        maybeSlider("lens_film_sl0", function (x) {
            if (film)
                film.set_arg0(x);
        });
        maybeSlider("lens_film_sl1", function (x) {
            if (film)
                film.set_arg1(x);
        });


        let film_exposure = maybeDrawer("lens_film_exposure", "film_exposure");
        maybeSlider("lens_film_exposure_sl0", function (x) {
            if (film_exposure)
                film_exposure.set_arg0(x);
        });
        maybeSlider("lens_film_exposure_sl1", function (x) {
            if (film_exposure)
        ...
        let basic = maybeDrawer("lens_basic", "basic");
        maybeSlider("lens_basic_sl0", function (x) {
            if (basic)
                basic.set_arg0(x);
        });
        maybeSlider("lens_basic_sl1", function (x) {
            if (basic)
                basic.set_arg1(x);
        });


        let focus_demo = maybeDrawer("lens_focus_demo", "focus_demo");

        maybeSlider("lens_focus_demo_sl0", function (x) {
            if (focus_demo)
                focus_demo.set_arg0(x);
        }, undefined, 0);

        let focus_demo2 = maybeDrawer("lens_focus_demo2", "focus_demo2");
        maybeSlider("lens_focus_demo2_sl0", function (x) {
            if (focus_demo2)
                focus_demo2.set_arg0(x);
        }, undefined, 0);

        let focus_demo3 = maybeDrawer("lens_focus_demo3", "focus_demo3");
        maybeSlider("lens_focus_demo3_sl0", function (x) {
            if (focus_demo3)
                focus_demo3.set_arg0(x);
        }, undefined, 0);
        maybeSlider("lens_focus_demo3_sl1", function (x) {
            if (focus_demo3)
                focus_demo3.set_arg1(x);
        }, undefined, 0);

        let bokeh = maybeDrawer("lens_bokeh", "bokeh");
        maybeSlider("lens_bokeh_sl0", function (x) {
            if (bokeh)
                bokeh.set_arg0(x);
        }, undefined, 0);


        let hero = maybeDrawer("lens_hero", "hero");
        maybeSlider("lens_hero_sl0", function (x) {
            if (hero)
                hero.set_arg0(x);
        });
        maybeSlider("lens_hero_sl1", function (x) {
            if (hero)
                hero.set_arg1(x);
        });
        maybeSlider("lens_hero_sl2", function (x) {
            if (hero)
                hero.set_arg2(x);
        });


        let film_invert = maybeDrawer("lens_film_invert", "film_invert");


        let frustum = maybeDrawer("lens_frustum", "frustum");
        maybeSlider("lens_frustum_sl0", function (x) {
            if (frustum)
                frustum.set_arg0(x);
        });

        field = maybeDrawer("lens_field", "field");
        field_slider = maybeSlider("lens_field_sl0", function (x) {
            if (field)
                field.set_arg0(x);
        });


        let field2 = maybeDrawer("lens_field2", "field2");
        maybeSlider("lens_field2_sl0", function (x) {
            if (field2)
                field2.set_arg0(x);
        });

        maybeSlider("lens_field2_sl1", function (x) {
            if (field2)
                field2.set_arg1(x);
        });

        let pixel_view = maybeDrawer("lens_hole_pixel_view", "pixel_view");
        maybeSlider("lens_hole_pixel_view_sl0", function (x) {
            if (pixel_view)
                pixel_view.set_arg0(x);
        });


        let hole_solid_angle = maybeDrawer("lens_hole_solid_angle", "hole_solid_angle");
        maybeSlider("lens_hole_solid_angle_sl0", function (x) {
            if (hole_solid_angle)
                hole_solid_angle.set_arg0(x);
        });

        let lens_solid_angle = maybeDrawer("lens_lens_solid_angle", "lens_solid_angle");
        maybeSlider("lens_lens_solid_angle_sl0", function (x) {
            if (lens_solid_angle)
                lens_solid_angle.set_arg0(x);
        });

        maybeSlider("lens_lens_solid_angle_sl1", function (x) {
            if (lens_solid_angle)
                lens_solid_angle.set_arg1(x);
        });

        let lens_solid_angle2 = maybeDrawer("lens_lens_solid_angle2", "lens_solid_angle2");
        maybeSlider("lens_lens_solid_angle2_sl0", function (x) {
            if (lens_solid_angle2)
                lens_solid_angle2.set_arg0(x);
        }, undefined, 0);

        lens_hole_sharpness = maybeDrawer("lens_hole_sharpness", "hole_sharpness");

        lens_hole_sharpness_slider = maybeSlider("lens_hole_sharpness_sl0", function (x) {
            if (lens_hole_sharpness)
                lens_hole_sharpness.set_arg0(x);
        });


        glass = maybeDrawer("lens_glass", "glass");
        glass_slider = maybeSlider("lens_glass_sl0", function (x) {
            if (glass)
                glass.set_arg0(x);
        });

        glass_rays = maybeDrawer("lens_glass_rays", "glass_rays");

        glass_rays_slider_0 = maybeSlider("lens_glass_rays_sl0", function (x) {
            if (glass_rays)
                glass_rays.set_arg0(x);
        });
        glass_rays_slider_1 = maybeSlider("lens_glass_rays_sl1", function (x) {
            if (glass_rays)
                glass_rays.set_arg1(x);
        });


        let wave_2d = maybeDrawer("lens_wave_2d", "wave_2d");
        let wave_3d = maybeDrawer("lens_wave_3d", "wave_3d");
        let wave_rays = maybeDrawer("lens_wave_rays", "wave_rays");


        wave_glass = maybeDrawer("lens_wave_glass", "wave_glass");


        wave_glass2 = maybeDrawer("lens_wave_glass2", "wave_glass2");
        wave_glass2_slider0 = maybeSlider("lens_wave_glass2_sl0", function (x) {
            if (wave_glass2)
                wave_glass2.set_arg0(x);
        });
        wave_glass2_slider1 = maybeSlider("lens_wave_glass2_sl1", function (x) {
            if (wave_glass2)
                wave_glass2.set_arg1(x);
        });


        let prism = maybeDrawer("lens_prism", "prism");

        maybeSlider("lens_prism_sl0", function (x) {
            if (prism)
                prism.set_arg0(x);
        });

        let parallel = maybeDrawer("lens_parallel", "parallel");

        maybeSlider("lens_parallel_sl0", function (x) {
            if (parallel)
                parallel.set_arg0(x);
        });

        subdiv = maybeDrawer("lens_subdiv", "subdiv");

        subdiv_seg = maybeSegmentedControl("lens_subdiv_seg0", function (x) {
            if (subdiv)
                subdiv.set_arg0(x);
        },
            ["3", "5", "7", "∞"]
        ...
        let fs = maybeDrawer("lens_f", "f");

        maybeSegmentedControl("lens_f_seg0", function (x) {
            if (fs)
                fs.set_arg0(x);
        },
        ...
        maybeDrawer("lens_device", "device");
        maybeDrawer("lens_rotational", "rotational");
        let spherical = maybeDrawer("lens_spherical", "spherical");
        maybeSlider("lens_spherical_sl0", function (x) {
            if (spherical)
                spherical.set_arg0(x);
        });

        let chromatic = maybeDrawer("lens_chromatic", "chromatic");
        maybeSlider("lens_chromatic_sl0", function (x) {
            if (chromatic)
                chromatic.set_arg0(x);
        });

        let focal_length = maybeDrawer("lens_focal_length", "focal_length");

        maybeSlider("lens_focal_length_sl0", function (x) {
            if (focal_length)
                focal_length.set_arg0(x);
        });
        maybeSlider("lens_focal_length_sl1", function (x) {
            if (focal_length)
                focal_length.set_arg1(x);
        });

        let lens_shape = maybeDrawer("lens_lens_shape", "focal_length");

        maybeSlider("lens_lens_shape_sl0", function (x) {
            if (lens_shape)
                lens_shape.set_arg0(x);
        });
        maybeSlider("lens_lens_shape_sl1", function (x) {
            if (lens_shape)
                lens_shape.set_arg1(x);
        });

        let sine = maybeDrawer("lens_sine", "sine");

        maybeSlider("lens_sine_sl0", function (x) {
            if (sine)
                sine.set_arg0(x);
        });
        maybeSlider("lens_sine_sl1", function (x) {
            if (sine)
                sine.set_arg1(x);
        }, undefined, 0);

        let em = maybeDrawer("lens_em", "em");




        rotational_focal = maybeDrawer("lens_rotational_focal", "rotational_focal");
        rotational_focal_slider = maybeSlider("lens_rotational_focal_sl0", function (x) {
            if (rotational_focal)
        ...
        snell_w = maybeDrawer("lens_snell", "snell");

        snell_slider0 = maybeSlider("lens_snell_sl0", function (x) {
            if (snell_w)
                snell_w.set_arg0(x);
        });

        snell_slider1 = maybeSlider("lens_snell_sl1", function (x) {
            if (snell_w)
                snell_w.set_arg1(x);
        }, undefined, 0);

        snell_slider2 = maybeSlider("lens_snell_sl2", function (x) {
            if (snell_w)
                snell_w.set_arg2(x);
        });


        snell2 = maybeDrawer("lens_snell2", "snell2");

        snell_slider20 = maybeSlider("lens_snell2_sl0", function (x) {
            if (snell2)
                snell2.set_arg0(x);
        });

        snell_slider21 = maybeSlider("lens_snell2_sl1", function (x) {
            if (snell2)
                snell2.set_arg1(x);
        }, undefined, 0);

        snell_slider22 = maybeSlider("lens_snell2_sl2", function (x) {
            if (snell2)
                snell2.set_arg2(x);
        });
        ...
        let cone_angle = maybeDrawer("lens_cone_angle", "cone_angle");

        maybeSlider("lens_cone_angle_sl0", function (x) {
            if (cone_angle)
                cone_angle.set_arg0(x);
        });

        let cone = maybeDrawer("lens_cone", "cone_angle");

        maybeSlider("lens_cone_sl0", function (x) {
            if (cone)
                cone.set_arg0(x);
        });


        let cone_slice = maybeDrawer("lens_cone_slice", "cone_slice");

        maybeSlider("lens_cone_slice_sl0", function (x) {
            if (cone_slice)
        ...
        let cone_aperture = maybeDrawer("lens_cone_aperture", "cone_aperture");

        maybeSlider("lens_cone_aperture_sl0", function (x) {
            if (cone_aperture)
                cone_aperture.set_arg0(x);
        });

        let cone_hex = maybeDrawer("lens_cone_hex", "cone_hex");

        maybeSlider("lens_cone_hex_sl0", function (x) {
            if (cone_hex)
                cone_hex.set_arg0(x);
        });

        cone_dof = maybeDrawer("lens_cone_dof", "cone_dof");
        cone_dof_slider0 = maybeSlider("lens_cone_dof_sl0", function (x) {
            if (cone_dof)
                cone_dof.set_arg0(x);
        });
        cone_dof_slider1 = maybeSlider("lens_cone_dof_sl1", function (x) {
            if (cone_dof)
                cone_dof.set_arg1(x);
        });
        cone_dof_slider2 = maybeSlider("lens_cone_dof_sl2", function (x) {
            if (cone_dof)
                cone_dof.set_arg2(x);
        });
        ...
        let blades = maybeDrawer("lens_blades", "blades");

        maybeSlider("lens_blades_sl0", function (x) {
            if (blades)
                blades.set_arg0(x);
        }, undefined, 0);
...
function global_animate(animate) {
    ...
    let on = document.getElementById("global_animate_on");
    let off = document.getElementById("global_animate_off");
